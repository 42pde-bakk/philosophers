...
/*   Updated: 2020/08/19 00:41:07 by peer          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (pthread_mutex_lock(&phil->data->pen))
		printf("pen mutex_lock error\n");
	...
	ft_putstr_fd(s, 1, 0);
	ft_putchar_fd('\n', 1);
	if (pthread_mutex_unlock(&phil->data->pen))
		printf("pen mutex_lock error\n");
}
...
void	*philosopher_death(t_philo *phil)
{
	...
	if (pthread_mutex_unlock(phil->lfork_mutex))
		printf("mutex_unlock error\n");
	if (pthread_mutex_unlock(phil->rfork_mutex))
		printf("mutex_unlock error\n");
	phil->state = DEAD;
	return (NULL);
}
...
void	*start_philosopher(void *param)
{
	...
	while (eatcount != phil->data->eat_times)
	{
		...
		if (pthread_mutex_lock(phil->lfork_mutex))
			printf("mutex_lock error\n");
		philosopher_write(phil, "has taken fork");
		if (pthread_mutex_lock(phil->rfork_mutex))
			printf("mutex_lock error\n");
		philosopher_write(phil, "has taken fork");
		// if (get_time_ms() - phil->last_ate >= (unsigned long)phil->data->time_to_die)
		// 	return (philosopher_death(phil));
		// pthread_mutex_lock(&phil->time_elapsed_mutex);
		phil->last_ate = get_time_ms();
		// pthread_mutex_unlock(&phil->time_elapsed_mutex);
